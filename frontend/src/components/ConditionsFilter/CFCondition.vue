...
function getValueControl() {
  ...
  } else if (field === '_assign' && ['like', 'not like'].includes(operator)) {
    return h(Link, {
      class: 'form-control',
      doctype: 'User',
      value: typeof condition[2] === 'string' ? condition[2].replace(/%/g, '') : condition[2],
      onChange: (v) => {
        condition[2] = v ? `%${v}%` : ''
      },
    })
...
function getOperators() {
  ...
  if (fieldname === '_assign') {
    options = [
      { label: 'Includes User', value: 'like' },
      { label: 'Does Not Include User', value: 'not like' },
      { label: 'Is', value: 'is' },
    ]
  }
