<template>
  <div class="flex flex-col h-full overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto p-5 sm:px-10">
      ...
      <div v-else class="max-w-6xl mx-auto pb-10">
        <!-- New Activity Dashboard -->
        <UserActivityDashboard ref="dashboardRef" :user="selectedUser" />

        <!-- Activity Feed Header -->
        <div class="flex items-center justify-between mb-4 mt-6">
          <h3 class="text-base font-bold text-ink-gray-9 flex items-center gap-2">
            <LucideActivity class="size-4 text-ink-gray-5" />
            {{ __('Chronological Activity Feed') }}
          </h3>
        </div>

        <div v-if="userActivities.loading" class="flex flex-col items-center justify-center py-12 text-ink-gray-4 gap-2 bg-surface-white border border-outline-gray-modals rounded-xl">
          <LoadingIndicator class="size-6 animate-spin" />
          <div>{{ __('Fetching activities...') }}</div>
        </div>
        <div v-else-if="!userActivities.data?.length" class="flex flex-col items-center justify-center py-12 text-ink-gray-4 gap-2 bg-surface-white border border-outline-gray-modals rounded-xl">
          <LucideActivity class="size-12 stroke-1" />
          <div class="text-lg font-medium">{{ __('No Activities Found') }}</div>
          <div class="text-sm text-ink-gray-5">{{ __('This user has not performed any timeline activities yet.') }}</div>
        </div>
...
import UserActivityDashboard from '@/components/UserActivityDashboard.vue'
