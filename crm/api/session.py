...
def get_users():
	...
	if hierarchy_enabled() and session_roles["is_sales_manager"]:
		allowed_reports = get_reporting_users(frappe.session.user)
		crm_users = [u for u in crm_users if u.name in allowed_reports]
		users = [u for u in users if u.name in allowed_reports]
