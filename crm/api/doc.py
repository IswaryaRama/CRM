...
def get_quick_filters(doctype: str, cached: bool = True):
	...
	if global_settings := frappe.db.exists("CRM Global Settings", {"dt": doctype, "type": "Quick Filters"}):
		...
		for filter in _quick_filters:
			...
			if not field:
				field = next((f for f in standard_fields if f.get("fieldname") == filter), None)
			if field:
				fields.append(field)
		...
		for std_field in standard_fields:
			if frappe.db.get_value("Property Setter", {"doc_type": doctype, "field_name": std_field["fieldname"], "property": "in_standard_filter"}, "value") == "1":
...
def get_data(
	...
	if doctype == "CRM Call Log":
		allowed_keys = ["telephony_medium", "type", "status", "from", "to", "call_date", "call_time", "caller", "receiver"]
