...
def get_public_url(path: str):
	...
	if hasattr(frappe.local, "request") and frappe.local.request:
		...
		if req_host:
			# If the request host is local (localhost, 127.0.0.1), check if site config has a public URL
			is_req_local = any(x in req_host for x in ["localhost", "127.0.0.1", "0.0.0.0"])
			if is_req_local:
				configured_url = get_url()
				is_config_local = any(x in configured_url for x in ["localhost", "127.0.0.1", "0.0.0.0"])
