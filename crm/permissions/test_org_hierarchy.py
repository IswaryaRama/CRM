...
class TestOrgHierarchy(IntegrationTestCase):
	...
	def test_assigner_can_read_lead(self):
		# Lead owned by outsider, assigned by rep1 to rep2
		lead = make_lead("outsider@hier.test")
		assign_todo("CRM Lead", lead.name, "rep2@hier.test", assigned_by="rep1@hier.test")
		# rep1 (who assigned it) should be able to read it
		self.assertTrue(has_lead_permission(lead, "read", "rep1@hier.test"))
...
def assign_todo(doctype, docname, allocated_to, status="Open", assigned_by=None):
	doc = frappe.get_doc(
		{
			"doctype": "ToDo",
