...
def clean_duration(duration, start_time=None, end_time=None):
	try:
		val = int(round(float(duration)))
		if start_time and end_time:
			st = frappe.utils.to_datetime(start_time)
			et = frappe.utils.to_datetime(end_time)
			if st and et:
...
def update_vobiz_call_status(call_sid: str, status: str, duration: int = 0):
	...
	if duration:
		call_log.duration = clean_duration(duration, call_log.start_time, call_log.end_time)
...
def hangup(**kwargs):
	...
	try:
		call_log.duration = clean_duration(duration, call_log.start_time or args.get("StartTime") or args.get("starttime"), call_log.end_time or args.get("EndTime") or args.get("endtime"))
	except ValueError:
		pass
