...
def get_mongo_uri():
	...
	# First try the docker0 gateway (most reliable from within bridge container)
	try:
		import socket
		s = socket.create_connection(("172.17.0.1", 27017), timeout=1)
		s.close()
	...
	for log in mongo_logs:
		try:
			...
			if lead_doc:
				...
				if not more_recent_exists:
					...
					if duration_val is not None:
						from crm.integrations.vobiz.api import clean_duration
						duration_val_int = clean_duration(
							duration_val,
							(log.get("timestamps") or {}).get("start"),
							(log.get("timestamps") or {}).get("end")
						)
			...
			if not call_log_exists:
				...
				from crm.integrations.vobiz.api import clean_duration
				call_log.duration = clean_duration(duration, start_time, end_time)
